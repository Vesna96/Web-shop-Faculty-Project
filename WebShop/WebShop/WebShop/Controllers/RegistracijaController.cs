...
namespace WebShop.Controllers
{
    public class RegistracijaController : Controller
    {
        ...
        [HttpPost]
        public ActionResult DodajKorisnika(RegistracijaModel rm, String potvrdaLozinke,String stilovi, String boje)
        {
            ...
            Korisnik k = new Korisnik(rm.Korisnik.Id,rm.Korisnik.Ime,rm.Korisnik.Prezime,rm.Korisnik.Pol,rm.Korisnik.DatumRodjenja,
                rm.Korisnik.Email,rm.Korisnik.Lozinka,rm.Korisnik.Adresa);

            client.Cypher.Create("(korisnik:Korisnik {k})")
                .WithParam("k", k)
            ...
            if (boje != "")
            {
                ...
                client.Cypher.Match("(korisnik:Korisnik)", "(boja:Boja)")
                    .Where((Korisnik korisnik) => korisnik.Email == rm.Korisnik.Email)
                    .AndWhere((Boja boja) => boja.Naziv == boje)
                    .Create("(korisnik)-[:OmiljenaBoja]->(boja)")
                    .ExecuteWithoutResults();

                List<Korisnik> listaKorisnika = new List<Korisnik>();
                listaKorisnika = client.Cypher.OptionalMatch("(korisnik:Korisnik) -[r:OmiljenaBoja]->(boja:Boja)")
                .Where((Boja boja) => boja.Naziv == boje)
                .Return(korisnik => korisnik.As<Korisnik>())
                .Results.ToList();

                foreach (Korisnik korisnik in listaKorisnika)
                {
                    if (rm.Korisnik.Email != korisnik.Email)
                    {
                        client.Cypher.Match("(k1:Korisnik)", "(k2:Korisnik)")
                            .Where((Korisnik k1) => k1.Email == rm.Korisnik.Email)
                            .AndWhere((Korisnik k2) => (k2.Email == korisnik.Email) && (k2.Pol == rm.Korisnik.Pol))
                            .Create("(k1) - [:istiPoBoji] - (k2)")
                            .ExecuteWithoutResults();
                    }
                }
            ...
            if(stilovi != "")
            {
                client.Cypher.Match("(korisnik:Korisnik)", "(stil:Stil)")
                   .Where((Korisnik korisnik) => korisnik.Email == rm.Korisnik.Email)
                   .AndWhere((Stil stil) => stil.Naziv == stilovi)
                   .Create("(korisnik)-[:StilOblacenja]->(stil)")
                   .ExecuteWithoutResults();

                List<Korisnik> listaKorisnika = new List<Korisnik>();
                listaKorisnika = client.Cypher.OptionalMatch("(korisnik:Korisnik) -[r:StilOblacenja]->(stil:Stil)")
                .Where((Stil stil) => stil.Naziv == stilovi)
                .Return(korisnik => korisnik.As<Korisnik>())
                .Results.ToList();
